          ...
          {processes.map(({ key, title, description, marker, Icon }) => (
            <li key={key}>
              <Card className='flex h-full flex-col gap-y-20 xl:gap-y-25'>
                ...
                >
                  <div className='border-secondary-200 hover:bg-primary-500 hover:text-primary-0 flex aspect-square min-h-20 items-center justify-center rounded-sm border p-2 transition-colors duration-300 xl:min-h-24 xl:rounded xl:border-2'>
                    <Icon width={20} height={20} className='xl:size-12' />
                  </div>
                  <span className='text-lg font-semibold xl:text-xl 2xl:text-2xl'>
                    {marker.toString().padStart(2, '0')}
                  </span>
                </div>

                <div className='flex flex-col gap-y-4 text-pretty'>
                  <h3 className='font-semibold xl:text-xl 2xl:text-2xl'>
                    {title}
                  </h3>
